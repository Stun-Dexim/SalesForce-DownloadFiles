...
def main():
    ...
    parser.add_argument('-q', '--query', required=True, help='SOQL query to select files.')
    parser.add_argument('-f', '--filenamepattern', default='{1}_{2}.{3}',
                        help='Filename pattern using indexed SOQL fields, default: {1}_{2}.{3}')
    ...
    # Check if credentials are pre-set in the ini file; prompt user if not
    ...
    # Initialize CSV metadata file with header  
    with open(results_path, 'w', encoding='utf-8', newline='') as f_csv:  
        writer = csv.writer(f_csv)  
        writer.writerow(metadata_header)  
    ...
    # Create a metadata dictionary indexed by unique record ID  
    metadata_dict = {}  
    ...
    # Write initial metadata file immediately (all rows with default failed status)  
    with open(results_path, 'w', encoding='utf-8', newline='') as f_csv:  
        writer = csv.writer(f_csv)  
        writer.writerow(metadata_header)  
        for row in metadata_dict.values():  
    ...
    # NEW: Identify Salesforce object from SOQL query  
    ...
    if salesforce_object == 'attachment':  
        ...
        logging.error(f"Unsupported Salesforce object: {salesforce_object}. Supported objects: Attachment, ContentDocument, ContentDocumentLink, ContentVersion.")  
        exit(1)  
      
    # NEW: Validate required fields are present  
    # Improved validation logic to handle nested fields correctly:  
    missing_fields = []  
    ...
    fetch_files(  
        ...
        metadata_header=metadata_header  # <-- Add this line explicitly  
    )   
